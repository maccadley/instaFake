...
//            captionLabel.text = post?.caption
    ...
    override init(frame: CGRect) {
        ...
        userNameLabel.anchor(top: topAnchor, left: userProfileImage.rightAnchor, bottom: photoImageView.topAnchor, right: optionsButton.leftAnchor, paddingTop: 0, paddingLeft: 8, paddingBottom: 0, paddingRight: 0, width: 0, height: 0)
    ...
    @objc func handleComment(){
        guard let postComment = post else {return}
        delegate?.didTapComment(post: postComment)
    ...
    fileprivate func setUpAttributedCaption(){
        ...
        //Here we create two empty line with size of 4. With this we get need space between caption text and duration
        attributedText.append(NSMutableAttributedString(string: "\n\n", attributes: [NSAttributedStringKey.font : UIFont.systemFont(ofSize: 4)]))
        let timeAgoDisplay = post.creationDate.timeAgoDisplay()
        attributedText.append(NSMutableAttributedString(string: timeAgoDisplay, attributes: [NSAttributedStringKey.font : UIFont.systemFont(ofSize: 14), NSAttributedStringKey.foregroundColor : UIColor.gray]))
        captionLabel.attributedText = attributedText
    }
    
    fileprivate func setUpAttributedUsername(){
        guard let post = self.post else {return}
        let attributedUsername = NSMutableAttributedString(string: post.user.username, attributes: [NSAttributedStringKey.font : UIFont.boldSystemFont(ofSize: 14), NSAttributedStringKey.foregroundColor : UIColor.black])
